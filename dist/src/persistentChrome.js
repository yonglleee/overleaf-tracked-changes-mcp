...
export function cdpPort(cdpUrl) {
...
async function activateExistingOverleafTab(cdpUrl) {
    try {
        const response = await fetch(new URL('/json/list', cdpUrl), {
...
export async function ensurePersistentChrome(input) {
    ...
    throw new Error(`Chrome started but ${cdpUrl} did not become available. Close Chrome using this profile and retry.`);
}
export function defaultPersistentProfileDirectory() {
    return process.env.OVERLEAF_BROWSER_PROFILE || path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'overleaf-tracked-changes-mcp', 'browser-profile');
}
//# sourceMappingURL=persistentChrome.js.map