...
export function projectIdFromUrl(projectUrl) {
    ...
    if (!match)
        throw new Error(`Overleaf project ID not found in URL: ${projectUrl}`);
    return match[1];
}
export function defaultSnapshotName(now = new Date()) {
    return `overleaf-snapshot-${now.toISOString().replace(/[:.]/g, '-')}`;
}
function resolveSnapshotDestination(destinationRoot, snapshotName) {
...
function inspectZipEntries(zip, maxExtractedBytes, maxEntries) {
    let files = 0;
    let directories = 0;
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory) {
            ...
            if (extractedBytes > maxExtractedBytes) {
                throw new Error(`Overleaf archive expands beyond the limit (${extractedBytes} bytes > ${maxExtractedBytes}).`);
            }
        }
    }
    return { files, directories, extractedBytes };
}
export async function extractProjectSnapshot(input) {
...
//# sourceMappingURL=projectSnapshot.js.map