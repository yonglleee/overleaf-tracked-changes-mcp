...
import { selectOpenFileName, trackedSnapshotChanged, } from '../src/overleafBrowser.js';
...
test('trackedSnapshotChanged detects only a new or changed tracked signal', () => {
    assert.equal(trackedSnapshotChanged({ count: 2, signature: 'same' }, { count: 2, signature: 'same' }), false);
    assert.equal(trackedSnapshotChanged({ count: 2, signature: 'same' }, { count: 3, signature: 'same' }), true);
    assert.equal(trackedSnapshotChanged({ count: 2, signature: 'before' }, { count: 2, signature: 'after' }), true);
});
//# sourceMappingURL=overleafBrowser.test.js.map