...
export function inspectLatexStructure(text: string, isLatex = true): DocumentPreflightOutput['syntax'] {
  ...
  for (let index = 0; index < source.length; index += 1) {
    if ((source[index] === '{' || source[index] === '}') && !isEscaped(source, index)) {
      if (source[index] === '{') braceStack.push(index);
  ...
  while ((match = environmentPattern.exec(source)) !== null) {
    ...
    if (kind === 'begin') {
      ...
      if (open?.name !== name) {
        errors.push(
          `Environment mismatch on line ${line}: opened ${open?.name} on line ${open?.line}, closed ${name}.`,
        );
      }
    }
...
async function compileLatex(
  ...
  compilePath: string,
  timeoutMs: number,
): Promise<DocumentPreflightOutput['compilation']> {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'overleaf-document-preflight-'));
  try {
    const engines: Array<{ name: string; args: string[] }> = [
      {
        name: 'latexmk',
...
export async function preflightLocalDocument(input: DocumentPreflightInput): Promise<DocumentPreflightOutput> {
