  ...
  switch (request.params.name) {
    case 'list_local_changes': {
      ...
      return textResult({
        baselineRoot,
        workingRoot,
        ...await listLocalChanges(
          baselineRoot,
          workingRoot,
          Number(args.max_entries || 5000),
          Number(args.max_bytes || 2_000_000),
        ),
      });
    }
    ...
    case 'sync_local_file_tracked': {
      ...
      if (hardBlocked || conflictBlocked || collaborativePlan.edits.length === 0) {
        ...
        return textResult({
          ...
          partial: false,
          blocked: !complete,
          reason: hardBlocked ? collaborativePlan.reason : (
            conflictBlocked ? 'conflicts_require_resolution' : 'no_safe_edits'
          ),
          dryRun: args.dry_run !== false,
          allowPartial,
          filePath: local.filePath,
    ...
    case 'get_overleaf_status': {
      return textResult(await browserClient.status(args.project_url as string | undefined));
    }
    ...
    case 'ensure_reviewing': {
      return textResult(await browserClient.ensureReviewing(args.project_url as string | undefined));
    }
...
async function runOpenCommand(): Promise<void> {
  const target = process.env.OVERLEAF_PROJECT_URL || 'https://www.overleaf.com/project';
  const launched = await ensurePersistentChrome({
    cdpUrl: defaultCdpUrl(),
    profileDirectory: defaultPersistentProfileDirectory(),
  ...
  await new Promise<void>((resolve) => {
    ...
    const finish = () => {
      ...
      if (input.isTTY && typeof input.setRawMode === 'function') input.setRawMode(Boolean(wasRaw));
      input.pause();
      resolve();
    };
...
async function runBrowserCommand(startUrl?: string): Promise<void> {
...
async function runSnapshotCommand(destinationRoot?: string, snapshotName?: string): Promise<void> {
  if (!destinationRoot) throw new Error('snapshot requires a destination root');
  const client = new OverleafBrowserClient();
  try {
...
async function runOpenFileCommand(filePath?: string): Promise<void> {
  if (!filePath) throw new Error('open-file requires a file path');
  const client = new OverleafBrowserClient();
  try {
...
async function runReviewingCommand(): Promise<void> {
  const client = new OverleafBrowserClient();
  try {
    console.log(JSON.stringify(
      await client.ensureReviewing(process.env.OVERLEAF_PROJECT_URL),
      null,
      2,
    ));
  } finally {
